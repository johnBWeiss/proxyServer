...
// console.time('multi')
// let testArray = multiple([{ link: 'https://www.google.com/' }, { link: 'https://www.google.com/' }, { link: 'https://www.google.com/', }])
...
async function screenShot(searchResults) {
    ...
    let image = await page.screenshot({ type: 'jpeg', quality: 5, omitBackground: true, encoding: 'base64' });
...
async function screenSho(searchResults) {
    ...
    const page = await browser.newPage();
    await page.goto(searchResults);
    let image = await page.screenshot({ type: 'jpeg', quality: 50, omitBackground: true, encoding: 'base64' });
    image = 'data:image/jpeg;base64,' + image
...
router.get('/google/:searchWord', async (req, res) => {
    ...
    // if (imagesMultiple[0].length > 1 && imagesMultiple[1].length > 1 && imagesMultiple[2].length > 1) {
    for (let i = 0; i < 3; i++) {
        console.time("loop")
